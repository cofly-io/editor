...
describe('prompt + repair message content', () => {
  it('system prompt contains the API card, rules, fixture, and version', () => {
    ...
    expect(prompt).toContain('DSL DEFINITION PATTERNS')
    expect(prompt).toContain('Pattern A: repeated linear parts along one axis')
    expect(prompt).toContain('Pattern B: surface-mounted accessory on a host face')
    expect(prompt).toContain('Pattern C: cylindrical body with ports and access details')
    expect(prompt).toContain('Pattern D: hinged door with child handle')
    expect(prompt).toContain('Pattern E: radial bolt circle')
    expect(prompt).toContain('Pattern F: arched bridge or repeated spans')
    expect(prompt).toContain('Pattern G: skid-mounted equipment package')
    expect(prompt).toContain('Pattern H: articulated chain')
