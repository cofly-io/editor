...
describe('reviewAssemblyRealism', () => {
  ...
  test('passes a semantic fan blower package generated by the equipment SDK', () => {
    const result = compileDsl(GOOD_FAN_BLOWER)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: GOOD_FAN_BLOWER })
    expect(review.applicable).toBe(true)
    expect(review.family).toBe('fan_blower')
    expect(review.passed).toBe(true)
    expect(review.evidence.semanticRoles).toEqual(
      expect.arrayContaining([
        'fan_volute_casing',
  ...
  test('rejects a fan blower approximated with raw primitives', () => {
    const result = compileDsl(RAW_FAN_BLOWER)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: RAW_FAN_BLOWER })
    expect(review.family).toBe('fan_blower')
    expect(review.passed).toBe(false)
    expect(review.issues).toEqual(
      expect.arrayContaining([
        expect.stringContaining('realism_missing_required_role'),
        expect.stringContaining('realism_material_preset_ratio'),
        expect.stringContaining('realism_fan_under_detailed'),
        expect.stringContaining('realism_fan_casing_faceting'),
        expect.stringContaining('realism_fan_impeller_blade_count'),
        expect.stringContaining('realism_fan_inlet_missing'),
        expect.stringContaining('realism_fan_bearing_missing'),
        expect.stringContaining('realism_fan_motor_missing'),
      ]),
    )
  })
  ...
  test('passes a semantic heat exchanger generated by the equipment SDK', () => {
    const result = compileDsl(GOOD_HEAT_EXCHANGER)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: GOOD_HEAT_EXCHANGER })
    expect(review.applicable).toBe(true)
    expect(review.family).toBe('heat_exchanger')
    expect(review.passed).toBe(true)
    expect(review.evidence.semanticRoles).toEqual(
      expect.arrayContaining(['heat_exchanger_shell', 'tube_sheet', 'tube_bundle']),
    )
  })

  test('rejects a heat exchanger approximated as a bare shell and two ports', () => {
    const result = compileDsl(RAW_HEAT_EXCHANGER)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: RAW_HEAT_EXCHANGER })
    expect(review.family).toBe('heat_exchanger')
    expect(review.passed).toBe(false)
    expect(review.issues).toEqual(
      expect.arrayContaining([
        expect.stringContaining('realism_missing_required_role'),
        expect.stringContaining('realism_material_preset_ratio'),
        expect.stringContaining('realism_heat_exchanger_under_detailed'),
        expect.stringContaining('realism_heat_exchanger_shell_faceting'),
        expect.stringContaining('realism_heat_exchanger_tube_sheets_missing'),
        expect.stringContaining('realism_heat_exchanger_tube_bundle_missing'),
        expect.stringContaining('realism_heat_exchanger_saddles_missing'),
        expect.stringContaining('realism_heat_exchanger_ports_missing'),
      ]),
    )
  })

  test('passes a semantic agitated reactor tank generated by the equipment SDK', () => {
    const result = compileDsl(GOOD_AGITATOR_TANK)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: GOOD_AGITATOR_TANK })
    expect(review.applicable).toBe(true)
    expect(review.family).toBe('agitated_vessel')
    expect(review.passed).toBe(true)
    expect(review.evidence.semanticRoles).toEqual(
      expect.arrayContaining([
  ...
  test('rejects an agitated reactor approximated as a tank with a small top cylinder', () => {
    const result = compileDsl(RAW_AGITATOR_TANK)
    expect(result.ok).toBe(true)
    if (!result.ok) return

    const review = reviewAssemblyRealism(result.ir, { source: RAW_AGITATOR_TANK })
    expect(review.family).toBe('agitated_vessel')
    expect(review.passed).toBe(false)
    expect(review.issues).toEqual(
      expect.arrayContaining([
        expect.stringContaining('realism_missing_required_role'),
        expect.stringContaining('realism_material_preset_ratio'),
        expect.stringContaining('realism_agitated_vessel_under_detailed'),
        expect.stringContaining('realism_agitated_vessel_shell_faceting'),
        expect.stringContaining('realism_agitated_vessel_heads_missing'),
        expect.stringContaining('realism_agitated_vessel_gearbox_missing'),
        expect.stringContaining('realism_agitated_vessel_shaft_missing'),
        expect.stringContaining('realism_agitated_vessel_impeller_missing'),
        expect.stringContaining('realism_agitated_vessel_ports_missing'),
        expect.stringContaining('realism_agitated_vessel_manhole_missing'),
        expect.stringContaining('realism_agitated_vessel_support_missing'),
      ]),
    )
  })
})
