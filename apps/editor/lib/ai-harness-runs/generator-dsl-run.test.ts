...
import { resolveGenerationMode, NO_SIGNALS } from './generation-route'
...
describe('executeGeneratorDslRun', () => {
  it('happy path: valid IR + gate pass → patches with root first and generator provenance', async () => {
    const result = await executeGeneratorDslRun(
      input({ runSandboxImpl: fakeSandbox(okResult), now: (() => { let t = 0; return () => (t += 60) })() }),
    )
    ...
    const result = await executeGeneratorDslRun(
      input({
        runSandboxImpl: fakeSandbox({
          kind: 'diagnostics',
          diagnostics: [
            { code: 'dsl_forbidden_global', severity: 'error', message: 'process is not available', retryable: false },
          ],
    ...
    const collapsed: AssemblyIR = {
      ...
      parts: LAPTOP_IR.parts.map((p) =>
        p.id.startsWith('keyboard.key.')
          ? { ...p, transform: { ...p.transform, position: [0, 0.024, 0] as [number, number, number] } }
          : p,
      ),
    }
    const result = await executeGeneratorDslRun(
