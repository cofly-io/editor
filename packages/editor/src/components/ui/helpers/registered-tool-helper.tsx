...
export function RegisteredToolHelper({ hints }: { hints: ToolHint[] }) {
  ...
  return (
    <div className="pointer-events-none fixed top-1/2 right-4 z-40 flex -translate-y-1/2 flex-col gap-1.5 rounded-lg border border-white/10 bg-neutral-950/90 px-3 py-2.5 text-white shadow-2xl backdrop-blur-md">
      {hints.map((hint) => (
        <div className="flex items-center gap-1.5 text-[11px]" key={`${hint.key}:${hint.label}`}>
          <ShortcutToken
            className="h-5 min-w-7 justify-center border-white/10 bg-white/5 px-1.5 text-[10px] text-neutral-300"
            value={hint.key}
          />
