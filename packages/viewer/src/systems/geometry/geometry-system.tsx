...
type GeometryDefinitionExtras = {
  geometryKey?: (node: AnyNode) => string
...
export const GeometrySystem = () => {
  ...
  const renderSettingsKey = `${shading}|${textures}|${colorPreset}|${sceneTheme}`

  useEffect(() => {
    void renderSettingsKey
  ...
  useFrame(() => {
    ...
    for (const id of dirtyIds) {
      ...
      const defExtras = def as typeof def & GeometryDefinitionExtras
      if (defExtras.geometryKey) {
        const builtKey = `${shading}|${textures}|${colorPreset}|${sceneTheme}|${defExtras.geometryKey(effectiveNode)}`
