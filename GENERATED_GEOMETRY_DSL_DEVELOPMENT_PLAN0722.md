...
6. 新增通用空间校验：重复实例的最小唯一位置数、同 ID/同 role 的意外完全重叠、无效变换、附件轴与 rotation 的矛盾。
   > **批注（2026-07-26）**：经评审，executor 层的诊断已能阻断"静默降级产物"进入 stage3，本项作为兜底防御**推迟到阶段 6 统一设计**（与阶段 6 工作项 4 的通用质量门合并实施，避免两处各做一半）。阶段 1 视为已收尾。
...
- 质量门不能仅因角色齐全而放行"全重叠重复件"或"零角度 lid"。（实现位置：阶段 6 通用质量门，见工作项 6 批注）
